...
# LowController motor IDs and documented mechanical position limits. Public
# card inputs use degrees; MotorCmd.pos is always sent in radians.
_BUMI_ARM_JOINTS = {
    "left": (
...
# Fixed, vendor-example-derived gains used internally by arm control. They are
# deliberately not exposed through the MCP card. Non-arm joints hold the pose
# measured at trajectory start because LowController requires all 21 commands.
_BUMI_LOWCONTROL_KP = (
    12.0, 12.0, 12.0, 12.0,
...
class ArmPlugin:
    ...
    def get_tool(self) -> dict:
        ...
        return {
            ...
            "description": (
                ...
                "(walking), which confirms that Bumi completed its standing preparation. "
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "speed_deg_s": {
                        ...
                        "description": (
                            "Maximum joint speed in degrees per second. Range 5-30; default "
                            "20. Lower values produce a slower arm movement."
                        ),
                    },
                },
                "anyOf": [
    ...
    def _validate_request(self, args: dict) -> tuple[dict, float] | dict:
        ...
        if isinstance(speed, bool) or not isinstance(speed, (int, float)):
            return {
                "state": "error", "command_sent": False,
                "error": "speed_deg_s must be a number from 5 to 30",
            }
        speed = float(speed)
        if not math.isfinite(speed) or not self._MIN_SPEED_DEG_S <= speed <= self._MAX_SPEED_DEG_S:
            return {
                "state": "error", "command_sent": False,
                "error": "speed_deg_s must be between 5 and 30",
            }
    ...
    @staticmethod
    def _read_positions(low_ctrl) -> list[float]:
        ...
        if not all(math.isfinite(position) for position in positions):
            raise RuntimeError("LowController returned a non-finite joint position")
        return positions
    ...
    def _move(self, args: dict) -> dict:
        ...
        if self._high_ctrl is None:
            return {
                "state": "error", "command_sent": False,
                "error": (
                    "Cannot verify the required workmode because HighController is "
                    "unavailable; no LowController command was sent."
                ),
            }
        try:
            workmode = int(self._high_ctrl.get_mode())
        except Exception as exc:
            return {
                "state": "error", "command_sent": False,
                "error": (
                    "Failed to read the required HighController workmode; no "
                    f"LowController command was sent: {exc}"
                ),
            }
        if workmode == 26:
            return {
                "state": "error", "command_sent": False,
                "current_workmode": workmode,
                "current_workmode_name": "protection",
                "error": (
                    "Robot is in protection mode. Restart and inspect the robot before "
        ...
        if workmode != 2:
            return {
                "state": "error", "command_sent": False,
                "current_workmode": workmode,
                "current_workmode_name": _WORKMODE_NAMES.get(workmode, "unknown"),
                "required_workmode": 2,
                "required_workmode_name": "walking",
                "error": (
                    "Arm movement is allowed only in workmode=2 (walking). Complete the "
                    "normal standing preparation before retrying; the arm card does not "
        ...
        try:
            ...
            start_positions = self._read_positions(self._low_ctrl)
            ...
            time.sleep(self._FEEDBACK_SETTLE_S)
            measured_positions = self._read_positions(self._low_ctrl)
            errors_deg = {
                f"{side}_{joint_name}": round(math.degrees(
                    measured_positions[motor_id] - target_positions[motor_id]), 3)
                for side, values in requested.items()
                for _, (motor_id, joint_name, _, _) in zip(values, _BUMI_ARM_JOINTS[side])
            }
            ...
            return {
                ...
                "message": (
                    "Arm movement completed and feedback is within tolerance."
                    if completed else
                    "The trajectory was sent, but measured arm feedback did not reach "
                    "the requested position within tolerance. Keep the safety hanger "
                    "attached and inspect the robot before retrying."
                ),
            }
        except Exception as exc:
            return {
...
# HighController locomotion and predefined-action cards.
class LocoPlugin:
    PREFIX = "loco"
